...
for(const l of manifest.lessons){const src=fs.readFileSync(l.url,'utf8');assert.equal((src.match(/class="exercise"/g)||[]).length,['B01','B02'].includes(l.id) ? 7 : 3,l.id);assert(src.includes('EXEMPLO RESOLVIDO A'));assert(src.includes('EXEMPLO RESOLVIDO B'));assert(fs.statSync(l.url).size<50000,`Aula demasiado grande: ${l.id}`);}
