...
for(const l of manifest.lessons){const src=fs.readFileSync(l.url,'utf8');assert.equal((src.match(/class="exercise"/g)||[]).length,3,l.id);assert(src.includes('EXEMPLO RESOLVIDO A'));assert(src.includes('EXEMPLO RESOLVIDO B'));assert(fs.statSync(l.url).size<30000,`Aula demasiado grande: ${l.id}`);}
...
console.log(`OK: ${html.length} páginas HTML, ${links} ligações locais, 38 aulas, 114 exercícios e cálculos de referência.`);
