...
const b4=lessons.find(l=>l.id==='B04');
const mean=xs=>xs.reduce((sum,x)=>sum+x,0)/xs.length;
const variance=xs=>mean(xs.map(x=>(x-mean(xs))**2));
assert.deepEqual(b4.questions.map(q=>q.a),[3/10*100,mean([2,4,9]),(4+8)/2,9-1,1,Math.round(variance([2,4,6])*100)/100,(20*100+80*200)/100]);
assert.equal(mean([100,100,100,500]),200);
assert.equal(variance([5,5,5]),0);
assert.equal(variance([2,4,6])*3/2,4);
